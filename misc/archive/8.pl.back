...
#  }
    ...
    print "Source is $sourceLength, and target is $targetLength\n";

    my $percentShorter = sprintf( "%.f", $target / $source * 100 );
    if $percentShorter < 0.60 {
        print "target\($targetLength\) is $ratio of source\($sourceLength\)\n";
      }
